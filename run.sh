...
COMPUTE_JOB_ID=$(sbatch --ntasks=$MPI_PROCESSES --cpus-per-task=$OMP_THREADS --mem-per-cpu=$MEM_PER_CPU --account=$ACCOUNT compute.slurm $NUM_PI_DIGITS)
echo ""
...
REDUCE_JOB_ID=$(sbatch --dependency=afterok:${COMPUTE_JOB_ID##* } --ntasks=1 --cpus-per-task=1 --mem-per-cpu=4gb --account=$ACCOUNT reduce.slurm $NUM_PI_DIGITS $MPI_PROCESSES)
